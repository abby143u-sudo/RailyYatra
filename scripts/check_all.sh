...
backend_status=0
migration_status=0
migration_runner_status=0
ingestion_status=0
import_status=0
frontend_status=0

printf '%s\n' '' '=== 1/6 Backend smoke test ==='
...
printf '%s\n' '' '=== 3/6 Migration runner dry-run ==='
python3 scripts/run_migrations.py --dry-run || migration_runner_status=$?
if [ "$migration_runner_status" -eq 0 ]; then
  printf '%s\n' 'PASS: migration runner dry-run'
else
  printf 'FAIL: migration runner dry-run exited with status %s\n' \
    "$migration_runner_status" >&2
fi

printf '%s\n' '' '=== 4/6 Ingestion smoke test ==='
...
printf '%s\n' '' '=== 5/6 Dry-run railway data import ==='
...
printf 'FAIL: backend=%s, migration=%s, runner=%s, ingestion=%s, import=%s, frontend=%s\n' \
  "$backend_status" "$migration_status" "$migration_runner_status" \
  "$ingestion_status" "$import_status" "$frontend_status" >&2
exit 1
