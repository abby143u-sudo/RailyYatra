...
COMPONENT_PATH = REPO_ROOT / "frontend" / "src" / "components" / "Phase5ProductStatusPanel.jsx"
APP_PATH = REPO_ROOT / "frontend" / "src" / "App.jsx"


def main() -> int:
    print("RailYatra frontend product status smoke test")
    ...
    required_component_phrases = [
        ...
        "real route demo is possible",
        "live booking claims are blocked",
    ]
    ...
    print("Frontend product status component checks:")
    ...
    if missing:
        print("FAIL: missing required product status frontend phrase(s)")
        for phrase in missing:
            print(f"  missing: {phrase}")
        return 1

    if missing_app:
        print("FAIL: App.jsx missing Phase5ProductStatusPanel wiring")
        for phrase in missing_app:
            print(f"  missing: {phrase}")
        return 1

    print("Frontend product status endpoint: /product/status")
    print("Live booking claim blocked: yes")
    print("Production railway tables modified: no")
    print("PASS: frontend product status smoke test completed")
    return 0
