...
function App() {
  ...
  const [apiStatus, setApiStatus] = useState({
    ...
    detail: "Testing RailBay API connection...",
  });
  ...
  async function submitManualFare(event) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.railbay.xyz/fare/manual?${params.toString()}`,
        { method: "POST" }
      );
  ...
  async function checkFareCoverage(event) {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.railbay.xyz/fare/lookup?${params.toString()}`
      );
  ...
  function buildJourneyShareText(item) {
    ...
    lines.push("RailBay Journey Option");
    lines.push("------------------------");
    ...
    if (item.type === "multi_transfer" && data.train_legs?.length) {
      ...
      data.train_legs.forEach((leg, index) => {
        lines.push(
          `${index + 1}. ${leg?.from} → ${leg?.to} by ${(leg?.train_no || leg?.train_number)} ${(leg?.train_name || leg?.name || "RailBay Journey Option") || ""}`
        );
      });
    }

    lines.push("------------------------");
    lines.push("Generated by RailBay");
  ...
  function getSmartBookingChecklist(item) {
    ...
    checks.push({
      ...
      detail: hasFare
        ? "RailBay has fare estimate/coverage for this option."
  ...
  function mapHealthPayloadToApiStatus(payload, statusCode = 200) {
    ...
    if (payload?.status === "ok") {
      return {
        ...
        detail: routing.message || api.message || "RailBay API and routing engine are connected.",
        meta: graphMeta,
      };
    }
  ...
  function buildSearchErrorDetails(error, statusCode = null) {
    ...
    if (lowerMessage.includes("failed to fetch") || lowerMessage.includes("networkerror")) {
      return {
        ...
        detail: "Frontend could not connect to RailBay API. Start backend server and retry.",
    ...
    return (
      <div className="clean-no-results-card">
        ...
        <div>
          ...
          <strong>RailBay could not find a route for this search</strong>
          <p>
    ...
    return (
      <div className="route-skeleton-panel">
        <div className="route-skeleton-heading">
          ...
          <strong>Finding smart RailBay options...</strong>
        </div>
    ...
    return (
      <div className="empty-suggestion-panel">
        <div>
          ...
          <p>RailBay found routes, but current filters are hiding them.</p>
      ...
      <div className="station-code-helper-panel">
        <div className="station-code-helper-heading">
          <div>
            ...
            <strong>Common RailBay test codes</strong>
          </div>
        </div>
        ...
        <button
          ...
          onClick={async () => {
            setApiStatus({
              ...
              detail: "Testing RailBay health endpoint...",
              meta: "",
            });
  ...
  function buildRecommendationReportText() {
    ...
    lines.push("RailBay Route Comparison Report");
    lines.push("=================================");
    lines.push(`Route: ${source} → ${destination}`);
    lines.push(`Class: ${journeyClass}`);
    if (maxFare) lines.push(`Max fare: ₹${maxFare}`);
    ...
    recommendations.forEach((item, index) => {
      ...
      if (item.type === "multi_transfer" && data.train_legs?.length) {
        ...
        data.train_legs.forEach((leg, legIndex) => {
          lines.push(
            `${legIndex + 1}. ${leg?.from} → ${leg?.to} by ${(leg?.train_no || leg?.train_number)} ${(leg?.train_name || leg?.name || "RailBay Journey Option") || ""}`
          );
        });
      }
    ...
    return (
      <div className="print-report">
        <div className="print-report-header">
          <h1>RailBay Route Comparison Report</h1>
                ...
                <div className="print-route-note">
                  {data.train_legs.map((leg, legIndex) => (
                    <div key={legIndex}>
                      ...
                      {(leg?.train_name || leg?.name || "RailBay Journey Option") || ""}
                    </div>
                  ))}
                </div>
              )}
  ...
  function renderDirectCard(item, index) {
    ...
    return (
      <div className="journey-card" key={cardId}>
        ...
        <h3>{(train?.train_no || train?.train_number)} — {(train?.train_name || train?.name || "RailBay Journey Option")}</h3>
        ...
        {expandedCard === cardId && (
          <div className="details-panel">
            ...
            <div className="detail-grid">
              ...
              <strong>{(train?.train_name || train?.name || "RailBay Journey Option")}</strong>
            ...
            <ul>
              {route.train_legs?.map((leg, i) => (
                <li key={i}>
                  ...
                  {(leg?.train_name || leg?.name || "RailBay Journey Option") || "Train"} · {safeValue(leg?.start_time)} →{" "}
